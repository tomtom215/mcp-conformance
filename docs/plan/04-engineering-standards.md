...
| Cross-architecture | `s390x` (qemu) + `i686` (native) | The engine crates' suites run on `s390x` (64-bit big-endian, under qemu) and `i686` (32-bit, native multilib) via `cargo xtask cross-arch`: the canonical form, the JSON/`JUnit` reports, and the golden corpus must be byte-identical where every CI host is 64-bit little-endian (`x86-64`/`aarch64`) — covering both endianness and pointer width. A byte-for-byte divergence is a failure. Scheduled (`cross-arch` job) |
