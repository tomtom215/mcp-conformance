...
| 1.5a | RC change inventory with SEP attributions: handshake removal (SEP-2575); `Mcp-Session-Id` removal (SEP-2567); required `Mcp-Method`/`Mcp-Name` Streamable-HTTP headers (SEP-2243); tool `inputSchema`/`outputSchema` lifted to full JSON Schema 2020-12 (SEP-2106); missing-resource error code `-32002` → JSON-RPC standard `-32602` (SEP-2164); `ttlMs`/`cacheScope` on list and resource-read results (SEP-2549); W3C Trace Context in `_meta` documented (SEP-414); ≥ 12 months deprecation-to-removal (SEP-2596). The RC states the **ten-week** window "is for SDK maintainers and client implementers to validate the changes against real workloads" and corroborates SEP-2484: "A Standards Track SEP can no longer reach Final status until a matching scenario lands in the conformance suite" | Verified | 2026-06-09 | RC announcement | Roadmap (draft-revision registry scope), Risks R1 |
...
| 2.4 | npm package: latest stable **0.1.16** (2026-03-30); **0.2.0-alpha** line active (alpha.2, 2026-06-03); ~20,985 downloads/month | Verified | 2026-06-10 | [npm registry](https://registry.npmjs.org/@modelcontextprotocol%2Fconformance) | Standards §pinning, Risks |
