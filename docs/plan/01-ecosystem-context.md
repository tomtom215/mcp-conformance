...
| 2.16 | Conformance-repo motion not yet on npm (all post-`alpha.2`): draft wire-version identifier renamed **`DRAFT-2026-v1` → `2026-07-28`** ([PR #331](https://github.com/modelcontextprotocol/conformance/pull/331), merged 2026-06-08) — README now reads: dated versions through `2025-11-25` use the stateful lifecycle, "the 2026 draft (`2026-07-28`) uses the stateless lifecycle (per-request `_meta`)" · python-sdk added as a built-in SDK runner target (PR #329, 2026-06-08) · `0.2.0-alpha.3` version bump committed 2026-06-08 but unpublished — npm `alpha` dist-tag still `0.2.0-alpha.2` on 2026-06-11 | Verified | 2026-06-11 | [Commits feed](https://github.com/modelcontextprotocol/conformance/commits/main.atom); [PR #331 diff](https://github.com/modelcontextprotocol/conformance/pull/331); [npm registry](https://registry.npmjs.org/@modelcontextprotocol%2Fconformance) | Suite tracking (alpha job), Roadmap M2.5 (our `draft-2026-07-28` gate name already matches the wire identifier; no in-repo references to the old `DRAFT-2026-v1` existed) |
...
| 3.8 | rmcp 1.7.0 client-side **drops `enumNames`** round-tripping elicitation schemas: untagged `EnumSchema` deserialization matches the legacy form as `Untitled` first (serialization is correct; only receiving rmcp clients lose the field) | Verified | 2026-06-10 | Duplex round-trip test in `mcp-everything-server` (`tests/roundtrip.rs` pins the loss) | Engagement (upstream bug report) |
