...
- [ ] `2026-07-28` registry entries extracted from the **final** spec text by the same
      ...
      *(In progress since 2026-08-06, and the box stays unchecked until every in-scope
      page is entered. Done: `basic/index` (57 clauses) and
      `basic/transports/streamable-http` (60) — a 117-entry registry, 51 judged /
      0 unsupported / 66 excluded, behind `draft-2026-07-28`, with `spec-drift`
      verifying all 257 quotes across both revisions and a passing plus a violating
      corpus trace behind every judged clause. Method and per-area decisions:
      [the extraction report](../reports/registry-extraction-2026-07-28-inventory-2026-08-06.md),
      whose addendums are the running log; the clause inventory in it is the backlog
      of the thirteen pages left. `out_of_scope` in
