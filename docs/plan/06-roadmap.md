...
| M5 — Stewardship artifacts | In progress — the rmcp tier-gap report is published ([docs/reports/rmcp-tier-gap-2025-11-25.md](../reports/rmcp-tier-gap-2025-11-25.md): 38/40 server scenarios at rmcp head `266f870`, re-measured live 2026-06-13); the mdBook is **built and CI-gated** (2026-06-13; [`book/`](../../book): five chapters across all four required areas, the trace-format and corpus chapters embedded verbatim from canonical sources via `{{#include}}`, with `mdbook build book` run on every push by the `book` CI job) — only the owner-gated GitHub Pages deployment that makes it "live" remains, and docs.rs completeness is enforced (`missing_docs = "deny"` + the `--all-features` rustdoc gate); the optional `pmcp` report and the `draft-2026-07-28` feature-gate drop remain |
