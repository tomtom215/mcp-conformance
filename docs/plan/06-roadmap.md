...
| M5 — Stewardship artifacts | In progress — the rmcp tier-gap report is published ([docs/reports/rmcp-tier-gap-2025-11-25.md](../reports/rmcp-tier-gap-2025-11-25.md): 38/40 server scenarios at rmcp head `266f870`, re-measured live 2026-06-13); the mdBook is **built and CI-gated** (2026-06-13; [`book/`](../../book): five chapters across all four required areas, the trace-format and corpus chapters embedded verbatim from canonical sources via `{{#include}}`, with `mdbook build book` run on every push by the `book` CI job) — the GitHub Pages deploy is now wired (`pages.yml`, all actions SHA-pinned; Pages enabled 2026-06-14), so the book goes live on the first `main` push that touches its sources, and docs.rs completeness is enforced (`missing_docs = "deny"` + the `--all-features` rustdoc gate); the optional `pmcp` report and the `draft-2026-07-28` feature-gate drop remain |
...
- [ ] mdBook live (architecture, trace format, corpus guide, conformance results page);
      ...
      *(2026-06-13: the book is built and gated, the "live" deployment is the only
      ...
      `-D warnings` (the `doc` CI job). The remaining piece is now wired:
      GitHub Pages is enabled (2026-06-14) and `pages.yml` (all actions
      SHA-pinned) builds and deploys the book; a new `workflow_dispatch` workflow
      cannot be triggered until it lands on the default branch, so the book goes
      **live on the first `main` push** that touches its sources — the one
      owner-controlled step left.)*
