...
| Suite tracking | Scheduled CI | Pinned-stable upgrades as deliberate PRs; `0.2.0-alpha` watched non-blocking. **Known gap (recorded 2026-07-26, not yet built):** the alpha job floats only the *suite binary* (`MCP_SUITE_VERSION=alpha`) — `xtask`'s `SPEC_VERSION` is a hard constant at `2025-11-25` with no override, so the job proves the alpha suite still passes our server *at the old revision* and gives **zero early warning on the `2026-07-28` scenario set**. Closing it is a small `MCP_SPEC_VERSION` override plus a second informational leg, and is the natural first move the day the final text ships |
