...
| Draft readiness | Scheduled CI (ratcheted) | `cargo xtask draft-readiness` drives the official runner's **`2026-07-28` draft scenarios** against the current server and compares against the committed [`conformance/draft-readiness.json`](../../conformance/draft-readiness.json). Both pins exact. First measurement 2026-07-26: **1 passing / 20 failing / 1 informational across 20 scenarios**, every failure the same HTTP 422 at the removed handshake — write-up in [docs/reports/draft-2026-07-28-readiness-2026-07-26.md](../reports/draft-2026-07-28-readiness-2026-07-26.md). The gate fails on any change in either direction, so M2.5's progress meter cannot drift silently |
